...
function M.parse_filter(query, docsets, entries)
  ...
  for _, name in ipairs(docset_names) do
    ...
    for _, ds in ipairs(docsets) do
      if ds.slug:lower() == lower or ds.name:lower() == lower or (ds.title or ""):lower() == lower then
        if not docset_set[ds.slug] then
          docset_set[ds.slug] = true
          table.insert(matched_docsets, ds.slug)
        end
      end
    end
  end
...
function M.filter_entries(entries, filters)
  ...
  return vim.tbl_filter(function(e)
    local match_docset = next(docset_set) == nil
      or docset_set[e.docset.slug:lower()]
